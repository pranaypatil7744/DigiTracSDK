package com.example.digitracksdk.presentation.attendance.view_attendance
...
import com.example.digitracksdk.databinding.ActivityViewAttendanceBinding
...
class ViewAttendanceActivity : BaseActivity(), ViewAttendanceAdapter.ViewAttendanceClickManager,
    ValidationListener {
    ...
    private fun setUpObserver() {
        binding.apply {
            with(attendanceViewModel) {
                viewAttendanceResponseData.observe(this@ViewAttendanceActivity) {
                ...
                messageData.observe(this@ViewAttendanceActivity) {
                    showToast(it)
                    showNoDataLayout(true)
                }
                progressBar.observe(this@ViewAttendanceActivity){
                    toggleLoader(it)
                }
            }
            ...
            with(attendanceRegularizationViewModel) {
                ...
                insertAttendanceRegularizationResponseData.observe(this@ViewAttendanceActivity) {
                    toggleLoader(false)
                    if (it.Status == Constant.SUCCESS) {
                        showToast(it.Message.toString())
            ...
            with(leavesViewModel) {
                ...
                leavesTypeResponse.observe(this@ViewAttendanceActivity) {
                    toggleLoader(false)
    ...
    private fun setUpListener() {
        binding.apply {
            layoutNoInternet.btnTryAgain.setOnClickListener {
                callViewAttendanceApi(fromDate = fromDate, toData = toDate)
            }
    ...
    private fun setUpView() {
        binding.apply {
            toolbar.apply {
                ...
                tvTitle.text = getString(R.string.view_attendance)
        ...
        bottomSheetNewAttendanceRegularizationBinding.apply {
            ...
            request.InTime =
                if (etInTime.text.toString().trim().isEmpty()) "00:00" else etInTime.text.toString()
                    .trim()
            request.Location = etLocation.text.toString().trim().ifEmpty { "null" }
            request.OutTime = etOutTime.text.toString().trim().ifEmpty { "00:00" }
    ...
    private fun openAttendanceRegularizationBottomSheet() {
        ...
        bottomSheetNewAttendanceRegularizationBinding.apply {
            ...
            etInTime.setOnClickListener {
                AppUtils.INSTANCE?.openTimePicker(etInTime, this@ViewAttendanceActivity)
            }
            etOutTime.setOnClickListener {
                AppUtils.INSTANCE?.openTimePicker(etOutTime, this@ViewAttendanceActivity)
            }
        }
    }
